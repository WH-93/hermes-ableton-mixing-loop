...
@register("get_all_device_parameters")
def get_all_device_parameters(song, params):
    """Return all parameters for all interesting devices on all tracks.
    One LOM traversal instead of N — reduces scan from 150s to ~3s.
    Filters to interesting device types for mixing (EQ Eight, Compressor, etc.)."""
    interesting = frozenset([
        "EQ Eight", "Compressor", "Compressor2", "Glue Compressor",
        "Saturator", "Utility", "Drum Buss", "Auto Filter", "AutoFilter2",
        "Operator", "Wavetable", "Analog", "Reverb", "Echo", "Delay",
        "Filter Delay", "Grain Delay", "Channel EQ", "EQ Three",
        "Chorus-Ensemble", "Phaser-Flanger", "Audio Effect Rack",
    ])
    ...
    for ti, track in enumerate(song.tracks):
        ...
        for di, device in enumerate(track.devices):
            if device.name not in interesting and device.class_name not in interesting:
                continue
            parameters = []
